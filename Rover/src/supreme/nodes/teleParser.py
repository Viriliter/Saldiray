        ...
        try:
            ...
            if r_byte.encode('hex')[:2] == 'fe':
                ...
                if r_byte.encode('hex')[2:4] == '02':
                    msg_len = int(r_byte.encode('hex')[4:6], 16)
                    for i in range (msg_len):
						lat1 = int(r_byte.encode('hex')[6+i*28:8+i*28], 16)
						lat2 = int(r_byte.encode('hex')[8+i*28:10+i*28], 16)
						lat3 = int(r_byte.encode('hex')[10+i*28:12+i*28], 16)
						lat4 = int(r_byte.encode('hex')[12+i*28:14+i*28], 16)
						lat5 = int(r_byte.encode('hex')[14+i*28:16+i*28], 16)
						lng1 = int(r_byte.encode('hex')[16+i*28:18+i*28], 16)
						lng2 = int(r_byte.encode('hex')[18+i*28:20+i*28], 16)
						lng3 = int(r_byte.encode('hex')[20+i*28:22+i*28], 16)
						lng4 = int(r_byte.encode('hex')[22+i*28:24+i*28], 16)
						lng5 = int(r_byte.encode('hex')[24+i*28:26+i*28], 16)
						alt1 = int(r_byte.encode('hex')[26+i*28:28+i*28], 16)
						alt2 = int(r_byte.encode('hex')[28+i*28:30+i*28], 16)
						alt3 = int(r_byte.encode('hex')[30+i*28:32+i*28], 16)
						#cmd  = int(r_byte.encode('hex')[32+i*32:34+i*32], 16)
						#wrad = int(r_byte.encode('hex')[34+i*32:36+i*32], 16)
						latArray[i]= (lat1+lat2*100+lat3*10000+lat4*1000000+lat5*100000000)/1000000.0
						lngArray[i]= (lng1+lng2*100+lng3*10000+lng4*1000000+lng5*100000000)/1000000.0
						altArray[i]= (alt1+alt2*100+alt3*10000)/100.0
						#cmdArray[i]= cmd
						#wradArray[i]= wrad
                    if msg_len>0:
                        output = str(">>>>>>>>>>>>>>>>>>>>>>>>>>>") + str(msg_len) + str(",") + str(latArray[:]) + "," + str(lngArray[:]) + "," + str(altArray[:])
                #print lat_int
                #print lon_int
            ...
            output = str("ERROR>>>>>>>>>>>>>>>>>>>>>>>>>") + str(msg_len) + str(",") + str(latArray[:]) + "," + str(lngArray[:]) + "," + str(altArray[:])
            pass
        return output
